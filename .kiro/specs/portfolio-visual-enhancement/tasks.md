...
- [ ] 1. Establecer la base del sistema de diseño
  - [ ] 1.1 Ampliar `tailwind.config.mjs` con los tokens de diseño
  ...
  - [ ] 1.2 Crear `src/styles/global.css` con reset, variables CSS y utilidades base
  ...
  - [ ] 1.3 Crear `src/utils/theme.ts` con las funciones puras de lógica de tema
  ...
  - [ ]* 1.4 Escribir pruebas de propiedad para las funciones de tema (Property 1 y 2)
    - **Property 1: Toggle de tema es reversible (round-trip)**
    - **Validates: Requirements 3.2, 3.4**
    - **Property 2: Persistencia del tema seleccionado**
    - **Validates: Requirements 3.4**
    - Crear `tests/theme.property.test.ts` con fast-check, `numRuns: 100`, usando `fc.constantFrom('dark', 'light')`
    - _Requirements: 3.2, 3.4_

- [ ] 2. Implementar el tema claro/oscuro sin parpadeo
  - [ ] 2.1 Modificar `src/layouts/Layout.astro` para inicialización flash-free de tema
  ...
  - [ ] 2.2 Crear el componente `src/components/ThemeToggle.astro`
  ...
  - [ ] 2.3 Integrar `ThemeToggle.astro` en `src/components/Header.astro`
...
- [ ] 3. Checkpoint — Asegurarse de que el toggle de tema funciona
...
- [ ] 4. Actualizar componentes para usar variables CSS del sistema de diseño
  - [ ] 4.1 Modificar `src/components/SectionContainer.astro`
  ...
  - [ ] 4.2 Modificar `src/components/SocialPill.astro`
  ...
  - [ ] 4.3 Modificar `src/components/ExperienceItem.astro`
  ...
  - [ ] 4.4 Modificar `src/components/AboutMe.astro`
  ...
  - [ ] 4.5 Modificar `src/components/Footer.astro`
    - Usar `var(--color-text-muted)` en el texto del footer
    - Aplicar `var(--color-surface)` / `var(--color-border)` en fondo y borde superior
    - _Requirements: 2.3, 3.5, 3.6_

- [ ] 5. Mejorar visualmente los proyectos
  - [ ] 5.1 Modificar `src/components/Projects.astro` con aspect-ratio uniforme y tokens de diseño
    ...
    - _Requirements: 2.3, 2.5, 5.1, 5.3, 5.4, 9.1, 9.3_

  - [ ]* 5.2 Escribir prueba de propiedad para campos requeridos de tarjetas (Property 3)
    - **Property 3: Tarjetas de proyecto contienen todos los campos requeridos**
    - **Validates: Requirements 5.1**
    - Usar `fc.constantFrom(...PROJECTS)` y `fc.constantFrom(...ALL_SELECTED_PROJECTS)` para iterar sobre todos los proyectos
    - Verificar que cada proyecto tiene `image.length > 0`, `title.length > 0`, `description.length > 0`, `tags.length > 0`, y al menos `github` o `preview` definido
    - Crear/ampliar `tests/projects.property.test.ts`; `numRuns: 100`
    - _Requirements: 5.1_

  - [ ]* 5.3 Escribir prueba de propiedad para aspect-ratio uniforme (Property 4)
    - **Property 4: Relación de aspecto uniforme en imágenes de tarjeta**
    - **Validates: Requirements 5.3**
    - Verificar que todos los proyectos tienen imagen con ruta no vacía (condición para que el `aspect-ratio` sea aplicable)
    - Usar `fc.constantFrom(...ALL_SELECTED_PROJECTS)` para todos los proyectos
    - Añadir al mismo archivo `tests/projects.property.test.ts`
    - _Requirements: 5.3_

  - [ ] 5.4 Modificar `src/pages/proyectos.astro` para grid multi-columna y tokens de diseño
  ...
  - [ ] 5.5 Modificar `src/pages/proyecto/[slug].astro` para usar tokens de diseño
    - Reemplazar colores hard-coded por variables CSS del sistema de diseño
...
- [ ] 6. Mejorar la sección Hero y animar con GSAP
  - [ ] 6.1 Modificar `src/pages/index.astro` (sección Hero) para usar tokens de diseño
    ...
    - _Requirements: 1.1, 2.2, 2.3, 4.1, 4.4, 8.1, 9.3_

  - [ ] 6.2 Modificar `src/components/Animations.astro` para compatibilidad con ViewTransitions y reduced-motion
  ...
  - [ ]* 6.3 Escribir prueba de propiedad para prefers-reduced-motion (Property 5)
    - **Property 5: Reducción de movimiento desactiva animaciones GSAP/Lenis**
    - **Validates: Requirements 6.2**
    - Mockear `window.matchMedia` para retornar `matches: true` en la query `(prefers-reduced-motion: reduce)`
    - Verificar que con `prefersReducedMotion === true` ninguna instancia de Lenis se crea ni se llama a `gsap.fromTo`
    - Añadir al archivo `tests/theme.property.test.ts` en una sección `describe` separada; `numRuns: 100`
    - _Requirements: 6.2_

- [ ] 7. Checkpoint — Verificar animaciones y diseño visual
  - Verificar que las animaciones Hero se completan en ≤ 1.4 segundos y respetan `prefers-reduced-motion`
  - Verificar que todas las secciones tienen los colores correctos en tema oscuro y tema claro
  - Asegurarse de que todos los tests pasan; consultar al usuario si surge alguna duda.

- [ ] 8. Responsividad, accesibilidad y rendimiento
  - [ ] 8.1 Verificar y corregir responsive layout en todas las páginas
    - Asegurar columna única en viewport < 768px en `index.astro`, `proyectos.astro` y `[slug].astro`
    - Asegurar que no hay `overflow-x` (scroll horizontal) en viewport 320–767px — revisar que ningún elemento tiene ancho fijo mayor al viewport
    - _Requirements: 7.1, 7.2_

  - [ ]* 8.2 Escribir prueba de propiedad para ausencia de overflow horizontal (Property 6)
    - **Property 6: Ausencia de desbordamiento horizontal en viewport móvil**
    - **Validates: Requirements 7.1**
    - Generar anchos de viewport entre 320 y 767px con `fc.integer({ min: 320, max: 767 })`
    - Verificar en JSDOM que `document.body.scrollWidth <= viewportWidth` para cada ancho generado
    - Añadir al archivo `tests/projects.property.test.ts`; `numRuns: 100`
    - _Requirements: 7.1_

  - [ ]* 8.3 Escribir prueba de propiedad para áreas táctiles (Property 7)
    - **Property 7: Áreas táctiles de elementos interactivos ≥ 44×44px**
    - **Validates: Requirements 7.4**
    - Generar elementos interactivos (`<a>`, `<button>`) desde la lista de componentes del portafolio
    - Verificar que `getBoundingClientRect().width >= 44` y `height >= 44` en un viewport táctil simulado
    - Añadir al archivo `tests/projects.property.test.ts`; `numRuns: 100`
    - _Requirements: 7.4_

  - [ ]* 8.4 Escribir prueba de propiedad para alt text en imágenes (Property 8)
    - **Property 8: Texto alternativo en todas las imágenes de contenido**
    - **Validates: Requirements 8.1**
    - Usar `fc.constantFrom(...ALL_SELECTED_PROJECTS)` para verificar que el `alt` generado (`Captura de pantalla de ${project.title}`) es una cadena no vacía para cualquier proyecto
    - Añadir al archivo `tests/projects.property.test.ts`; `numRuns: 100`
    - _Requirements: 8.1_

  - [ ]* 8.5 Escribir prueba de propiedad para lazy loading en imágenes (Property 9)
    - **Property 9: Carga diferida en imágenes fuera del viewport inicial**
    - **Validates: Requirements 9.1**
    - Usar `fc.constantFrom(...ALL_SELECTED_PROJECTS)` para verificar que la ruta de cada imagen comienza con `/` (imagen local en `public/`, lo que garantiza que se puede aplicar `loading="lazy"`)
    - Añadir al archivo `tests/projects.property.test.ts`; `numRuns: 100`
    - _Requirements: 9.1_

  - [ ]* 8.6 Escribir prueba de propiedad para espacio reservado en imágenes (Property 10)
    - **Property 10: Espacio reservado para imágenes (sin CLS)**
    - **Validates: Requirements 9.3**
    - Usar `fc.constantFrom(...ALL_SELECTED_PROJECTS)` para verificar que cada proyecto tiene `image.length > 0` (condición para que el contenedor con `aspect-ratio: 16/10` reserve espacio correctamente)
    - Añadir al archivo `tests/projects.property.test.ts`; `numRuns: 100`
    - _Requirements: 9.3_

- [ ] 9. Checkpoint final — Todos los tests deben pasar
  - Ejecutar `pnpm vitest --run` y confirmar que todos los tests de `tests/theme.property.test.ts` y `tests/projects.property.test.ts` pasan
  - Revisar visualmente el portafolio en viewport 375px (móvil) y 1280px (escritorio) en ambos temas
  - Asegurarse de que todos los tests pasan; consultar al usuario si surge alguna duda.
